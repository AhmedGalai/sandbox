...
# Intent classification examples with embeddings
INTENT_EXAMPLES = {
    "query_status": [
        "where is the robot",
        "what is the robot status",
        "show me robot position",
        "robot location",
        "check robot errors",
        "is robot ready"
    ],
    ...
    "task_complex": [
        "go to A then check status",
        "move to B and charge",
        "navigate to all points",
        "check robot then send to charge"
    ]
}
...
async def classify_intent(user_query: str) -> Dict[str, Any]:
    """Classify user intent using embeddings"""
    query_embedding = await get_embedding(user_query.lower())

    if not query_embedding:
        # Fallback to keyword matching
        query_lower = user_query.lower()
        if any(kw in query_lower for kw in ["where", "status", "position", "location", "error"]):
            ...
            return {"intent": "task_navigation", "confidence": 0.5}

    # Calculate similarity with each intent category
    best_intent = "task_navigation"
    best_score = 0.0

    for intent, examples in INTENT_EXAMPLES.items():
        scores = []
        for example in examples:
            example_embedding = await get_embedding(example)
...
async def update_robot_pose(pose: dict):
    """Update robot pose (for testing)"""
    global robot_pose
    robot_pose.update(pose)
    return {"success": True, "pose": robot_pose}

def calculate_nearest_label():
    """Find nearest work point to robot"""
    import math
    min_dist = float('inf')
    nearest = None

    for label, coords in work_points.items():
        dist = math.sqrt((coords['x'] - robot_pose['x'])**2 + (coords['y'] - robot_pose['y'])**2)
        if dist < min_dist:
            min_dist = dist
            nearest = label
...
async def get_robot_status():
    ...
    return {
        ...
        "current_task": robot_pose.get("current_task", "Idle"),
        "status": robot_pose.get("status", "Ready"),
        "errors": robot_pose.get("errors", "None"),
...
async def send_robot_task(task: RobotTask):
    ...
    try:
        ...
        robot_status_str = ""
        if task.context_data.get("robots"):
            robots_info = []
            ...
            if "then" in task_lower or "," in task_lower:
                ...
                for part in parts:
                    ...
                    for point in ["a", "b", "c", "charge", "standby"]:
                        if point in part or f"to {point}" in part:
                            step_target = point.upper()
                            break
                    if step_target:
                        steps.append({"action": "go", "poi": step_target})
