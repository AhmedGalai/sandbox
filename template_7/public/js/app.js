...
let robotMovementIntervals = {}; // Per-robot movement intervals
...
// Start robot movement with animation
function startRobotMovementForRobot(robotId) {
    const robot = robots[robotId];
    if (!robot || !robot.path || robot.path.length === 0) return;

    // Clear existing movement interval for this robot
    if (robotMovementIntervals[robotId]) {
        clearInterval(robotMovementIntervals[robotId]);
    }
    ...
    robotMovementIntervals[robotId] = setInterval(() => {
        if (robot.pathIndex >= robot.path.length) {
            // Reached destination
            clearInterval(robotMovementIntervals[robotId]);
            delete robotMovementIntervals[robotId];
            robot.isMoving = false;
            robot.status = 'Ready';
            robot.currentTask = 'Idle';
            robot.path = [];
            robot.targetPoint = null;
            ...
            // Update systems card
            updateExecutionStatus(`${robotId} completed task`);
            return;
        }
        ...
        if (dx !== 0 || dy !== 0) {
            robot.heading = Math.round(Math.atan2(-dy, dx) * 180 / Math.PI);
        ...
        }).catch(err => console.log('Failed to update server:', err));
...
async function sendRobotToPoint(robotId, pointLabel) {
    ...
    startRobotMovementForRobot(robotId);

    // Update systems card
    updateExecutionStatus(`${robotId} moving to ${pointLabel}...`);
}

// Wait for robot to finish moving
function waitForRobotToFinish(robotId) {
...
async function executeMultiStepTaskForRobot(robotId, steps) {
    ...
    for (let i = 0; i < steps.length; i++) {
        const step = steps[i];
        updateTaskPlan(`Executing step ${i+1}/${steps.length} for ${robotId}: ${step.action} ${step.poi || ''}`);

        if (step.action === 'go' && step.poi) {
            await sendRobotToPoint(robotId, step.poi);
            await waitForRobotToFinish(robotId);
        } else if (step.action === 'wait' && step.seconds) {
            updateExecutionStatus(`${robotId} waiting ${step.seconds} seconds...`);
            await new Promise(resolve => setTimeout(resolve, step.seconds * 1000));
        }
    }

    updateExecutionStatus(`${robotId} completed all steps`);
}

// Update systems card displays
function updateTaskPlan(planText) {
    const planSteps = document.getElementById('planSteps');
    if (planSteps) {
        const timestamp = new Date().toLocaleTimeString();
        planSteps.innerHTML = `<div style="margin-bottom: 5px; color: #666;">[${timestamp}]</div>${planText}`;
    }
}

function updateExecutionStatus(statusText) {
    const statusDetails = document.getElementById('statusDetails');
    if (statusDetails) {
        const timestamp = new Date().toLocaleTimeString();
        statusDetails.innerHTML = `<div style="margin-bottom: 5px; color: #666;">[${timestamp}]</div>${statusText}`;
    }
}
