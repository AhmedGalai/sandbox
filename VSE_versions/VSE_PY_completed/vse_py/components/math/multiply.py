"""Multiply component for arithmetic multiplication operations.

This module implements a simple multiplication processor component that multiplies
two float values together and outputs the result.

Example:
    >>> mul_comp = MultiplyComponent()
    >>> mul_comp.get_name()
    'Multiply'
"""
...
@register_component(
    type_name="MultiplyComponent",
    display_name="Multiply",
    category="Math",
    description="Multiplies two float values together (A * B)"
)
class MultiplyComponent(ProcessorComponent):
    """Component that multiplies two float inputs together.

    This processor takes two float input pins A and B, and outputs their
    product on the Result output pin.
    ...
    Attributes:
        ...
        Outputs:
            Result (float): Product of A and B (A * B)
    """
    ...
    def process(self) -> None:
        ...
        Retrieves values from inputs A and B, multiplies them together, and
        ...
        try:
            ...
            self.report_error(f"Unexpected error in Multiply component: {str(e)}")
