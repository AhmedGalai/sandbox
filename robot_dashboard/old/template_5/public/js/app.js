...
function handleRobotSelection() {
    ...
    // Update active robot to first selected
    if (selectedRobots.length > 0) {
        activeRobot = selectedRobots[0];
        robotState = robots[activeRobot];
    }

    // Redraw map with selected robots
    drawRobotMap();
}
...
async function executeTask() {
    ...
    if (action === 'go') {
        await sendToPoint(poi);
    } else if (action === 'lift') {
        const direction = document.querySelector('input[name="liftDirection"]:checked').value;
        document.getElementById('taskControlStatus').textContent = `Executing: Lift ${direction}`;
        document.getElementById('taskControlStatus').style.color = '#28a745';
        // Here you would send lift command to robot
        console.log(`Lift ${direction} at ${poi}`);
    } else if (action === 'wait') {
...
async function executeMultiStepTask() {
    ...
    document.getElementById('taskBuilderStatus').textContent = 'Executing task...';
    document.getElementById('taskBuilderStatus').style.color = '#28a745';

    for (let i = 0; i < taskSteps.length; i++) {
        const step = taskSteps[i];
        document.getElementById('taskBuilderStatus').textContent = `Step ${i + 1}/${taskSteps.length}: ${step.action} at ${step.poi}`;

        if (step.action === 'go') {
            await sendToPoint(step.poi);
            // Wait for robot to reach destination
            await new Promise(resolve => setTimeout(resolve, 2000));
        } else if (step.action === 'lift') {
            console.log(`Lift ${step.direction} at ${step.poi}`);
    ...
    document.getElementById('taskBuilderStatus').textContent = 'Task completed!';
}
