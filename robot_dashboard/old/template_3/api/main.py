...
async def send_robot_task(task: RobotTask):
    ...
    try:
        ...
        system_message = f"""Robot assistant. Be BRIEF.

Robot: ({robot_pose['x']},{robot_pose['y']}) heading {robot_pose['heading']}°
Points: A(18,3), B(7,12), C(16,16), CHARGE(2,18)
Nearest: {nearest}

Examples:
"go to A" -> "Moving to A"
"where are you" -> "At ({robot_pose['x']},{robot_pose['y']}) facing {robot_pose['heading']}°"
"status" -> "Position ({robot_pose['x']},{robot_pose['y']}), heading {robot_pose['heading']}°, nearest {nearest}"
...
Keep answers under 20 words."""
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            # Detect target point
            for point in ["a", "b", "c", "charge"]:
                if f"to {point}" in task_lower or f"point {point}" in task_lower or (point == "charge" and "charg" in task_lower):
